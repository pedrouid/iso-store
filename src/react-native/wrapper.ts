...
import { STORE_DEFAULT, STORE_KEY } from '../constants';
...
export class WrappedAsyncStorage implements KeyValueStorage {
  ...
  constructor(
    ...
    private readonly asyncStorageKey: string = STORE_KEY.STORE
  ) {
    this.loadData();
  }
  ...
  async fetch(): Promise<AsyncStorageData> {
    ...
    if (typeof json === 'string') {
      throw new Error(
        `Failed to fetch asyncstorage data for key: ${this.asyncStorageKey}`
      );
    }
    return json || {};
  }
