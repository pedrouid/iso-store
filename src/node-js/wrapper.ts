...
export class WrappedSequelizeStorage implements KeyValueStorage {
  ...
  constructor(
    ...
    this.IsoStoreData = this.sequelize.define(this.tableName, {
      key: {
  ...
  async getItem<T>(key: string): Promise<T | undefined> {
    try {
      const item = await this.IsoStoreData.findByPk(
  ...
  async setItem(key: string, value: any): Promise<void> {
    const execute = async (options = {}) => {
      try {
        await this.IsoStoreData.upsert(
          {
  ...
  async removeItem(key: string): Promise<void> {
    const execute = async (options = {}) => {
      try {
        await this.IsoStoreData.destroy(
  ...
  private async getRelevantItems(): Promise<any[]> {
    try {
      return this.IsoStoreData.findAll({
        where: {
          key: {
